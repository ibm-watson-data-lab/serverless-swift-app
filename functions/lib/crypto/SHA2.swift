...
final class SHA2/*: DigestType*/ {
    ...
    func calculate32() -> Array<UInt8> {
        // mw:added Utils().
        var tmpMessage = Utils().bitPadding(to: self.message, blockSize: 64, allowance: 64 / 8)
        ...
        // mw:tmpMessage += (message.count * 8).bytes(totalBytes: 64 / 8)
        tmpMessage += NumberExtensions().bytesFromInt(i: (message.count * 8), totalBytes: 64 / 8)
        
        // Process the message in successive 512-bit chunks:
        let chunkSizeBytes = 512 / 8 // 64
        // mw:for chunk in BytesSequence(chunkSize: chunkSizeBytes, data: tmpMessage) {
        let chunks = stride(from: 0, to: tmpMessage.count, by: chunkSizeBytes).map {
            Array(tmpMessage[$0..<min($0 + chunkSizeBytes, tmpMessage.count)])
        }
        let collectionExtensions = CollectionExtensions()
        let utils = Utils() // mw:
        for chunk in chunks {
            ...
            for x in 0..<M.count {
                ...
                case 0...15:
                    ...
                    let le = collectionExtensions.toUInt32Array(collection:chunk[start..<end])[0]
                    M[x] = le.bigEndian
                    break
                default:
                    let s0 = utils.rotateRight(M[x-15], by: 7) ^ utils.rotateRight(M[x-15], by: 18) ^ (M[x-15] >> 3)
                    let s1 = utils.rotateRight(M[x-2], by: 17) ^ utils.rotateRight(M[x-2], by: 19) ^ (M[x-2] >> 10)
            ...
            for j in 0..<variant.k.count {
                let s0 = utils.rotateRight(A, by: 2) ^ utils.rotateRight(A, by: 13) ^ utils.rotateRight(A, by: 22)
                let maj = (A & B) ^ (A & C) ^ (B & C)
                let t2 = s0 &+ maj
                let s1 = utils.rotateRight(E, by: 6) ^ utils.rotateRight(E, by: 11) ^ utils.rotateRight(E, by: 25)
                let ch = (E & F) ^ ((~E) & G)
    ...
    func calculate64() -> Array<UInt8> {
        // mw:added Utils().
        var tmpMessage = Utils().bitPadding(to: self.message, blockSize: 128, allowance: 128 / 8)
        ...
        // mw:tmpMessage += (message.count * 8).bytes(totalBytes: 128 / 8)
        tmpMessage += NumberExtensions().bytesFromInt(i: (message.count * 8), totalBytes: 128 / 8)
        ...
        for chunk in chunks {
            ...
            for x in 0..<M.count {
                ...
                case 0...15:
                    ...
                    let le = collectionExtensions.toUInt64Array(collection:chunk[start..<end])[0]
                    M[x] = le.bigEndian
                    break
                default:
                    let s0 = utils.rotateRight(M[x-15], by: 1) ^ utils.rotateRight(M[x-15], by: 8) ^ (M[x-15] >> 7)
                    let s1 = utils.rotateRight(M[x-2], by: 19) ^ utils.rotateRight(M[x-2], by: 61) ^ (M[x-2] >> 6)
                    M[x] = M[x-16] &+ s0 &+ M[x-7] &+ s1
                    break
                }
            }
            ...
            // Main loop
            for j in 0..<variant.k.count {
                let s0 = utils.rotateRight(A, by: 28) ^ utils.rotateRight(A, by: 34) ^ utils.rotateRight(A, by: 39) //FIXME: n:
                let maj = (A & B) ^ (A & C) ^ (B & C)
                let t2 = s0 &+ maj
                let s1 = utils.rotateRight(E, by: 14) ^ utils.rotateRight(E, by: 18) ^ utils.rotateRight(E, by: 41)
                let ch = (E & F) ^ ((~E) & G)
