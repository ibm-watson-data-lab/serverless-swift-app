...
// mw: Changed Error to Swift.Error
...
// mw:moved from below
enum LoadResult {
  case success(header:[String: Any], payload:[String: Any], signature:Data, signatureInput:String)
...
/// Decode a JWT
public func decode(_ jwt:String, algorithms:[Algorithm], verify:Bool = true, audience:String? = nil, issuer:String? = nil) throws -> [String: Any] {
  switch load(jwt) {
  case let .success(header, payload, signature, signatureInput):
    if verify {
      if let failure = Claims().validateClaims(payload, audience: audience, issuer: issuer) ?? verifySignature(algorithms, header: header, signingInput: signatureInput, signature: signature) {
        throw failure
      }
    }
...
/// Decode a JWT
public func decode(_ jwt:String, algorithm:Algorithm, verify:Bool = true, audience:String? = nil, issuer:String? = nil) throws -> [String: Any] {
  return try decode(jwt, algorithms: [algorithm], verify: verify, audience: audience, issuer: issuer)
}

// MARK: Parsing a JWT

// mw:moved out of new Decode class
// enum LoadResult {
//   case success(header:[String: Any], payload:[String: Any], signature:Data, signatureInput:String)
//   case failure(InvalidToken)
// }
...
func load(_ jwt:String) -> LoadResult {
  ...
  // mw:added Base64().
  let headerData = Base64().base64decode(headerSegment)
  if headerData == nil {
    return .failure(.decodeError("Header is not correctly encoded as base64"))
  }

  let header = (try? JSONSerialization.jsonObject(with: headerData!, options: JSONSerialization.ReadingOptions(rawValue: 0))) as? [String: Any]
  if header == nil {
    return .failure(.decodeError("Invalid header"))
  }

  // mw:added Base64().
  let payloadData = Base64().base64decode(payloadSegment)
  if payloadData == nil {
    return .failure(.decodeError("[String: Any] is not correctly encoded as base64"))
  }
  ...
  // mw:added Base64().
  let signature = Base64().base64decode(signatureSegment)
  if signature == nil {
...
func verifySignature(_ algorithms:[Algorithm], header:[String: Any], signingInput:String, signature:Data) -> InvalidToken? {
